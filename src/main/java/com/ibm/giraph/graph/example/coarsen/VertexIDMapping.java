...
public class VertexIDMapping implements Tool {
    ...
    static class MyMapper
	    extends
	    Mapper<LongWritable, SkeletonNeighborhood, LongWritable, LongWritable> {
	private int[] map;
	...
	public void map(LongWritable key, SkeletonNeighborhood value,
		Context context) throws IOException, InterruptedException {
	    // if(!map.containsKey((int) key.get()))
	    // throw new
	    // IOException("cannot find the mapping for node "+key.get());
	    context.write(key, new LongWritable(map[(int) key.get()]));
	}
    }
	...
	if (args.length < 4) {
	    System.err
		    .println("At least 3 arguments are requiered: <graph file> <recode mapping> <OutputPath> <#nodes>");
	    System.exit(-1);
	}
	Job job = new Job();
	...
	ToolRunner.run(new VertexIDMapping(), args);
    }
}
