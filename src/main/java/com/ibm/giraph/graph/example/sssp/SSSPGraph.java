...
public class SSSPGraph
	...
	while(iterator.hasNext())
	{
        ...
        while (hp.size() > 0)
        {
            ...
            //LOG.info("~~~ vid: " + cur.getVertexIdSimpleType() + " value: "  + cur.getVertexValueSimpleType()) ;
		...
		//LOG.info("\t!!! vid: " + vid.toString() + " edgeWeight : "  + edgeWeight ) ;
		LongDoubleDoubleDoubleVertex nvertex = (LongDoubleDoubleDoubleVertex) subgraph
			.getVertex(vid);

		if (nvertex == null) {
		    //LOG.info("\t### vid: " + cur.getVertexIdSimpleType() + " sends msg to " + vid.toString());
