...
public class RecodeGraphEdgeDoubleValue implements Tool {

	private Configuration conf;
	public static final String MAP_FILE_NAME="map.file.name";
	public static final String NUM_NODES="num.nodes";
	static class MyMapper extends Mapper<LongWritable, LongDoubleDoubleNeighborhood, 
	LongWritable, LongDoubleDoubleNeighborhood>
	{
		private int[] map;
		...
		public void map(LongWritable key, LongDoubleDoubleNeighborhood value, Context context)
	...
	public int run(String[] args) throws Exception {
		...
		job.setMapOutputValueClass(LongDoubleDoubleNeighborhood.class);
		job.setInputFormatClass(KVBinaryInputFormat.class);
		KVBinaryInputFormat.setInputNeighborhoodClass(job.getConfiguration(), LongDoubleDoubleNeighborhood.class);
		FileInputFormat.addInputPath(job, new Path(args[0]));
		job.getConfiguration().set(MAP_FILE_NAME, args[1]);
		Path outpath = new Path(args[2]);
		FileOutputFormat.setOutputPath(job, outpath);
		job.setJarByClass(RecodeGraphEdgeDoubleValue.class);
	...
	public static void main(String args[]) throws Exception {
		ToolRunner.run(new RecodeGraphEdgeDoubleValue(), args);
	}
}
