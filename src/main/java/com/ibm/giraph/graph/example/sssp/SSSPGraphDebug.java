...
public class SSSPGraphDebug
	...
	Iterator<BasicVertex<LongWritable, DoubleWritable, DoubleWritable, DoubleWritable>> it = subgraph
		.getVertices().iterator();

	while (it.hasNext()) {
	    LongDoubleDoubleDoubleVertex v = (LongDoubleDoubleDoubleVertex) it
		    .next();
	    int size = vidmap.size();
	    vidmap.put(v.getVertexIdSimpleType(), size + 1);
	}

	Iterator<LongDoubleDoubleDoubleVertex> iterator = innerVertices
		.iterator();

	QEle[] ele = new QEle[subgraph.getVertices().size() + 1];
	MinHeap hp = new MinHeap();
	while (iterator.hasNext()) {
	    LongDoubleDoubleDoubleVertex v = (LongDoubleDoubleDoubleVertex) iterator
		    .next();
	...
	while (hp.size() > 0) {
	    ...
	    // LOG.info("~~~ vid: " + cur.getVertexIdSimpleType() + " value: " +
	    // cur.getVertexValueSimpleType()) ;
	...
	while (it.hasNext()) {
	    LongDoubleDoubleDoubleVertex v = (LongDoubleDoubleDoubleVertex) it
		    .next();
	    LOG.info("~~~ vid: " + v.getVertexIdSimpleType() + " value: "
		    + v.getVertexValueSimpleType());
	}
    }

    private static class MyVertex extends LongDoubleDoubleDoubleVertex {
	@Override
	...
	GiraphJob job = new GiraphJob(getConf(), "SSSPGraphDebug");
	...
	System.exit(ToolRunner.run(new SSSPGraphDebug(), args));
    }
